...
def fit_to_curve(data, func, lowerbound=None, upperbound=None):
    ...
    params, covars = sp_opt.curve_fit(func,data[0,lower:upper],data[1,lower:upper],p0=[850,2.2])
...
def lab3_plotter(data,params):
    ...
    fit_ypts = expon_decay(fit_xpts,params[0],params[1])
...
def expon_decay(x,Coeff,Tau):
    return Coeff*np.exp(-1*x/Tau)
                ...
                try:
                    ...
                    if len(data)>0:
                        ...
                        try:
                            ...
                            channels = float(input("Avg. number of channels per period: "))
                                
                            if period>0 and channels>0:
                                scale_array = np.array([[channels],[period]])
                                ...
                                try:
                                    ...
                                    if lower<upper:
                                        ...
                                        print("Value Error: lower bound must be less than upper bound")
                                    
                                except:
                                    print("Value Error: bounds must be 2 numerical values")
                                    
                            else:
                                print("Value Error: period and channels must be positive, nonzero values")
                                
                        except:
                            print("Value Error: period and channels must be numerical values")
                            time.sleep(2)
